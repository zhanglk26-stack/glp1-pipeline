...
if (!weight || !height) return null;return (weight / ((height / 100) ** 2)).toFixed(1);}
function calculateMatch() {
const weight = parseFloat(document.getElementById('weight').value);const height = parseFloat(document.getElementById('height').value);const duration = parseInt(document.getElementById('duration').value);const conditions = Array.from(document.querySelectorAll('input[name="condition"]:checked')).map(cb => cb.value);if (!weight || !height) return alert('请输入完整的体重和身高信息');const bmi = calculateBMI(weight, height);const results = products.map(p => {
let score = 50;if (bmi >= p.bmi_range[0] && bmi <= p.bmi_range[1]) score += 20;const matchCond = conditions.filter(c => p.indications.includes(c));score += matchCond.length * 15;if (p.id === 'mazdutide' && conditions.includes('nash')) score += 10;return {
product: p,
score,
loss: p.weight_loss_curve.week48,
lossKg: (weight * p.weight_loss_curve.week48 / 100).toFixed(1)
};}).sort((a, b) => b.score - a.score).slice(0, 3);document.getElementById('results').classList.remove('hidden');document.getElementById('bmiValue').textContent = bmi;document.getElementById('targetWeight').textContent = `-${(weight * 0.15).toFixed(1)}kg`;document.getElementById('waistStatus').textContent = bmi > 28 ? '高风险' : '中等风险';renderCards(results);renderCostTable(results, duration);renderChart(results);document.getElementById('results').scrollIntoView({ behavior: 'smooth' });}
function renderCards(matches) {
const container = document.getElementById('recommendations');container.innerHTML = matches.map((m, i) => `
...
const months = duration / 4;document.getElementById('costTable').innerHTML = matches.map(m => `
<tr>
...
const ctx = document.getElementById('weightLossChart').getContext('2d');if (window.lossChart) window.lossChart.destroy();window.lossChart = new Chart(ctx, {
type: 'line',
data: {
